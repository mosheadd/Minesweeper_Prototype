...
int getBombsCount(short row, short col)
{
	int bombs_count = 0;
	...
	if (row >= 1 && row <= 8 && col >= 1 && col <= 8)
	{
		for (int t = row - 1; t < row + 2; t++)
			for (int k = col - 1; k < col + 2; k++)
				bombs_count += map[t][k] == MINE || map[t][k] == FLAGGED_MINE;
	}
	else if (col >= 1 && col <= 8)
	{
	...
	else
	{
		...
		bombs_count += map[row + coeff1][col + coeff2] == MINE + map[row][col + coeff2] == MINE + map[row + coeff1][col] == MINE;
		bombs_count += map[row + coeff1][col + coeff2] == FLAGGED_MINE + map[row][col + coeff2] == FLAGGED_MINE + map[row + coeff1][col] == FLAGGED_MINE;
	}
...
void makeRoad(short row, short col)
{
	...
	int cell_bombs_count = getBombsCount(row, col);

	map[row][col] = UNCOVERED;

	if (cell_bombs_count > 0) return;
	...
	while (true)
	{
		for (short i = 0; i < 10; i++)
		{
			for (short j = 0; j < 10; j++)
			{
				...
				{
					int bombs_count = getBombsCount(i, j);
		...
		bool game_won = checkGameWinCondition();
